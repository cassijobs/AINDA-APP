const app=document.querySelector('#app');let stream=null,arquivo='';
function inicio(){parar();app.innerHTML=`<section class="tela inicio"><span class="marca">AINDA · PROTÓTIPO</span><h1>Guarde este momento.</h1><p>Este teste abre a câmera frontal no próprio navegador. Uma frase da jornada acompanhará a fotografia, que guarda exatamente o enquadramento da câmera.</p><div class="acoes"><button class="botao principal" id="abrir">Abrir câmera frontal</button><a class="botao texto" href="../">Voltar</a></div></section>`;document.querySelector('#abrir').onclick=abrir}
async function abrir(){
  app.innerHTML=`<section class="camera"><video id="video" autoplay playsinline muted></video><canvas id="canvas"></canvas><header class="topo"><strong>GUARDE ESTE MOMENTO</strong><button class="fechar" id="fechar">×</button></header><div class="frase"><strong>Ainda há caminho.</strong><small>AINDA</small></div><div class="disparo"><button class="capturar" id="capturar" aria-label="Tirar selfie" disabled></button></div></section>`;
  document.querySelector('#fechar').onclick=inicio;
  document.querySelector('#capturar').onclick=capturar;
  try{
    parar();
    ...
    await new Promise((resolve,reject)=>{if(video.readyState>=2&&video.videoWidth)return resolve();const limite=setTimeout(()=>reject(new Error('A câmera abriu, mas não enviou imagem.')),8000);video.addEventListener('loadeddata',()=>{clearTimeout(limite);resolve()},{once:true})});
    await video.play();
    document.querySelector('#capturar').disabled=false;
  }catch(e){
    parar();
    alert(e?.message||'Não foi possível abrir a câmera frontal. Verifique a permissão da câmera.');
    inicio();
  }
